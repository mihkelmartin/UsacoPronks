    ...
    public static void main(String[] args )throws Exception {
        ...
        if(arve <= 5){
            ...
            for (int i = 1; i < arve + 1; i++) {
                arvud[i] = Byte.parseByte(arvud_str[i-1]);
            }
        } else {
            for (int i = 1; i < arve + 1; i++) {
                arvud[i] = Byte.parseByte(in.readLine());
            }
        }
        ...
        for (int i = 0; i < kaske; i++) {
            ...
            if(arvud_str[0].equals("M")){
                arvud[Integer.parseInt(arvud_str[1])] = Byte.parseByte(arvud_str[2]);
            } else {
                byte algus = Byte.parseByte(arvud_str[1]);
                byte lopp = Byte.parseByte(arvud_str[2]);
